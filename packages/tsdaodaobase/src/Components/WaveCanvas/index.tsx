...
export default class WaveCanvas extends Component<WaveCanvasProps> {
  ...
  cutAudioData() {
    ...
    if (waveformCount < waveformMaxNum) {
      // 当前波形小于最大波形则需要扩充
    ...
    } else if (waveformCount > waveformMaxNum) {
      // 当前波形大于最大波形则需要缩小
    ...
    for (let i = 0; i < filteredSamplesMA.length; i++) {
      filteredSamplesMA[i] =
        filteredSamplesMA[i] * scaleFactor < minHeight
          ? minHeight
          : filteredSamplesMA[i] * scaleFactor;
    }
  ...
  drawWaveform() {
    ...
    for (let index = 0; index < waveformList.length; index++) {
      ...
      ctx.fillRect(
        index * (this.barWidth() + this.barSpace()),
        canvasHeight - waveformList[index],
        this.barWidth(),
        waveformList[index]
      );
    }
