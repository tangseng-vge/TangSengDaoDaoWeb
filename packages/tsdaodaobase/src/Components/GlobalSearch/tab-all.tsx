...
export default class TabAll extends Component<TabAllProps> {
  ...
  render(): ReactNode {
    ...
    return (
      <div className="wk-tab-all" onScroll={this.handleScroll}>
        {existFriends ? (
          <Section title="联系人">
        ...
        {existGroups ? (
          <Section title="群组">
            {this.props.searchResult?.groups.map((item: any) => {
              if (
                this.props.keyword &&
                item.channel_name.indexOf(this.props.keyword) !== -1
              ) {
        ...
        {existMessages ? (
          <Section title="消息">
            {this.props.searchResult?.messages.map((item: any) => {
              ...
              let sender;
              if (
                item.channel.channel_type !== ChannelTypePerson &&
                item.from_uid &&
                item.from_uid !== ""
              ) {
                ...
                const channelInfo =
                  WKSDK.shared().channelManager.getChannelInfo(senderChannel);
                if (channelInfo) {
              ...
              return (
                <ItemMessage
                  ...
                  avatar={WKApp.shared.avatarChannel(
                    new Channel(
