...
export class ConversationWrap {
  ...
  public get avatar() {
    if (
      this.channelInfo &&
      this.channelInfo.logo &&
      this.channelInfo.logo !== ""
    ) {
      return `${WKApp.dataSource.commonDataSource.getImageURL(
        this.channelInfo.logo
      )}?v=${WKApp.shared.getChannelAvatarTag(this.channel)}`;
    }
  ...
  public get category() {
    if (
      !this.conversation.channelInfo ||
      !this.conversation.channelInfo.orgData
    ) {
      return "";
    }
    const channelInfo = this.conversation.channelInfo;
    if (
      channelInfo.orgData.category !== "" &&
      channelInfo.orgData.category === "solved"
    ) {
      return channelInfo.orgData.category;
    }
    if (
      channelInfo.orgData.category === "" &&
      channelInfo.orgData.agent_uid === ""
    ) {
      return "new";
    }
...
export class MessageWrap {
  ...
  private parseMention(): Array<Part> {
    ...
    if (
      this.message.remoteExtra.isEdit &&
      this.message.remoteExtra.contentEdit !== undefined
    ) {
