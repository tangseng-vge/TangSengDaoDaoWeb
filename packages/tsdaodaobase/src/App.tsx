...
export class ShowImages {
  ...
  // 批量添加图片
  addImages(imagesToAdd: StoredImageData[], channelId: string) {
    if (!imagesToAdd || imagesToAdd.length === 0 || !channelId) {
      return;
    }
  ...
  // 设置一个频道的图片列表
  setChannelImages(channelId: string, imagesToSet: StoredImageData[]) {
    if (!channelId) {
      return;
    }
    let allImages = this.getStorageItemForImages();
    // 移除这个频道以前的图片
    const otherChannelImages = allImages.filter(img => img.channelId !== channelId);
    // 添加新图片
...
export default class WKApp extends ProviderListener {
  ...
  baseContext!: WKBaseContext; // Bage基础上下文
